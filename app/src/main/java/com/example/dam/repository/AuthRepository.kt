...
class AuthRepository {
     ...
     * POST /auth/google
     * Works with existing backend - checks preferences after login
     */
    suspend fun googleSignIn(idToken: String): Result<GoogleSignInResponse> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                ...
                Log.d(TAG, "🔑 Access token (first 30 chars): ${googleResponse.access_token.take(30)}...")

                // Return the response - we'll check preferences separately
