...
class UserProfileRepository {
    ...
    suspend fun getFollowStats(userId: String, token: String): MyResult<AuthApiService.FollowStatsResponse> = withContext(Dispatchers.IO) {
        try {
            Log.d("UserProfileRepo", "========== GET FOLLOW STATS ==========")
            Log.d("UserProfileRepo", "UserId: $userId")
            Log.d("UserProfileRepo", "Token: ${formatToken(token).take(30)}...")

            val response = authApi.getFollowStats(userId, formatToken(token))

            Log.d("UserProfileRepo", "Response Code: ${response.code()}")
            Log.d("UserProfileRepo", "Response Body: ${response.body()}")

            if (response.isSuccessful && response.body() != null) {
                val stats = response.body()!!
                Log.d("UserProfileRepo", "✅ Stats loaded: followers=${stats.followers}, following=${stats.following}")
                MyResult.Success(stats)
            } else {
                val errorBody = response.errorBody()?.string()
                Log.e("UserProfileRepo", "❌ Failed to get follow stats: $errorBody")
                MyResult.Failure(Exception("Failed to get follow stats: ${response.code()}"))
            }
        } catch (e: Exception) {
            Log.e("UserProfileRepo", "❌ Exception getting stats: ${e.message}", e)
            MyResult.Failure(e)
        }
    }
    ...
    suspend fun getFollowing(userId: String, token: String): MyResult<FollowingResponse> = withContext(Dispatchers.IO) {
        try {
            val response = authApi.getFollowing(userId, token = formatToken(token))
            if (response.isSuccessful && response.body() != null) {
                MyResult.Success(response.body()!!)
            } else {
                MyResult.Failure(Exception("Failed to load following: ${response.code()}"))
            }
        } catch (e: Exception) {
