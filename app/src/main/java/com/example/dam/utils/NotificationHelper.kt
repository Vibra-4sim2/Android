...
object NotificationHelper {
    ...
    fun showNotification(context: Context, notification: Notification) {
        ...
        try {
            // ✅ Vérifier que le type n'est pas null (protection contre les types inconnus)
            ...
            with(NotificationManagerCompat.from(context)) {
                // ✅ Double vérification de la permission juste avant l'affichage
                if (hasNotificationPermission(context)) {
    ...
    private fun createDeepLinkIntent(context: Context, notification: Notification): Intent {
        val intent = Intent(context, MainActivity::class.java).apply {
            ...
            putExtra("notification_type", notification.type?.name ?: "UNKNOWN")
            ...
            when (notification.type) {
                ...
                null -> {
                    // Type inconnu, pas de données supplémentaires
                    Log.w(TAG, "⚠️ Notification type is null, no extra data added")
                }
            }
        }
